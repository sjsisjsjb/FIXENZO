...
export async function otakuDesu(search) {
  try {
...
function teradlx(url) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!match) {
                throw new Error('surl not found in the URL');
            }

            const surl = match[1];
            const forxins = `https://dm.terabox.app/indonesian/sharing/link?surl=${surl}`;

            const metadataResponse = await axios.get(`https://ins.neastooid.xyz/api/downloader/Metateradl?url=${encodeURIComponent(forxins)}`);
...
teradlx("https://www.terabox.app/wap/share/filelist?surl=qx26_6N3phpSAfwtefjT-g")
...
module.exports = {ffstalk, mlstalk, npmstalk, mlstalk, githubstalk, remini, pinterest, wallpaper, wikimedia, porno, hentai, quotesAnime, igstalk, aiovideodl, umma, ringtone, styletext }